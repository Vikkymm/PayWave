...
Creates DB, templates, static, uploads.
...
def deposit():
    ...
    try:
        ...
        flash("Enter a valid USD amount","danger"); return redirect(url_for("dashboard"))
    db = get_db()
    r = db.execute("SELECT rate_ngn_per_usd FROM rates WHERE method=?", (method,)).fetchone()
    if not r:
        flash("Invalid method","danger"); return redirect(url_for("dashboard"))
    amount_ngn = round(amount_usd_f * r["rate_ngn_per_usd"], 2)
    file = request.files.get("proof")
    filename = None
    if file and file.filename:
        if not allowed_file(file.filename):
            flash("File type not allowed","danger"); return redirect(url_for("dashboard"))
...
def withdraw():
    ...
    try:
        ...
        flash("Enter valid amount","danger"); return redirect(url_for("dashboard"))
    if not all([name,bank,account]):
        flash("Fill all withdrawal fields","danger"); return redirect(url_for("dashboard"))
    db = get_db()
    # balance check before inserting
    ...
    if amount_ngn_f > user["balance_ngn"]:
        ...
        return redirect(url_for("dashboard"))
    db.execute("INSERT INTO withdrawals (user_id,name,bank,account,amount_ngn,status,created_at) VALUES (?,?,?,?,?,?,?)",
...
def admin():
    ...
    rates = db.execute("SELECT * FROM rates").fetchall()
    trades = db.execute("SELECT t.*, u.email as user_email FROM trades t JOIN users u ON u.id=t.user_id ORDER BY t.created_at DESC").fetchall()
    withdrawals = db.execute("SELECT w.*, u.email as user_email FROM withdrawals w JOIN users u ON u.id = w.user_id ORDER BY w.created_at DESC").fetchall()
    totals = {
...
def approve_withdraw(wid):
    ...
    if w and w["status"]=="Pending":
        ...
        if user and user["balance_ngn"] >= w["amount_ngn"]:
            ...
            # can't approve: insufficient balance (shouldn't usually happen because we check at request time)
